...
function udl(x1,x2){ return {type:'udl', x1:x1, x2:x2}; }
function pt(x){ return {type:'point', x:x}; }
function tri(x1,x2,dir){ return {type:'tri', x1:x1, x2:x2, dir:dir}; } // dir 'up'|'down' = which end is tall
...
},
{
  ...
  supports:p=>[pin(0),pin(p.a+p.b+p.c)],
  loads:p=>[udl(0,p.a),udl(p.a+p.b,p.a+p.b+p.c)],
  totalLength:p=>p.a+p.b+p.c,
  calc(p){
...
},
{
  ...
  inputs:[{k:'L',l:'Span, L'},{k:'W',l:'Total load, W'}],
  supports:p=>[pin(0),pin(p.L)],
  loads:p=>[tri(0,p.L/2,'up'),tri(p.L/2,p.L,'down')],
  totalLength:p=>p.L,
  calc(p){
    const {L,W}=p, R=W/2;
...
},
{
  ...
  calc(p){
    ...
    const Vx=x=> x<=L1? R1-w*x : R3-w*(L1+L2-x);
...
},
{
  ...
  supports:p=>[pin(0),pin(p.L1),pin(p.L1+p.L2)],
  loads:p=>[pt(p.L1/2),pt(p.L1+p.L2/2)],
  totalLength:p=>p.L1+p.L2,
  calc(p){
